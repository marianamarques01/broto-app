  ...
  return (
    <div className="broto-week-strip">
      {rotina.map((dia, i) => {
        ...
        const dotColor =
          dia.ehDescanso || !dia.area ? 'var(--text-muted)' : getAreaColor(dia.area.value)

        return (
          <div key={dia.idx} className="broto-week-day">
            <div
              className={`broto-week-day__letter${dia.ehHoje ? ' broto-week-day__letter--today' : ''}`}
            >
              {dia.labelCurto}
            </div>
            <div
              className={`broto-week-day__num${dia.ehHoje ? ' broto-week-day__num--today' : dia.ehPassado ? ' broto-week-day__num--past' : ''}`}
            >
              {dayNum}
            </div>
            <div
