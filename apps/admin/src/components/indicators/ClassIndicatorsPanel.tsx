        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 14 }}>
          <thead>
            <tr style={{ background: 'var(--bg-deep)' }}>
              {['Aluno', 'Questoes', 'Acerto', 'Streak', 'XP', ''].map(h => (
                <th key={h} style={{ padding: '10px 20px', textAlign: 'left', fontWeight: 500, fontSize: 12, color: 'var(--text-muted)', borderBottom: '1px solid var(--border-default)' }}>
                  {h}
                </th>
              ))}
            </tr>
          ...
          <tbody>
            {indicators.students.length === 0 ? (
              <tr>
                <td colSpan={6} style={{ padding: '24px 20px', textAlign: 'center', color: 'var(--text-muted)', fontSize: 13 }}>
              ...
              <tr key={student.student_id} style={{ borderBottom: '1px solid var(--border-subtle)' }}>
                ...
                <td style={{ padding: '12px 20px', color: 'var(--text-secondary)' }}>{student.total_questions}</td>
                <td style={{ padding: '12px 20px' }}>
                  <span style={{
                    color: student.accuracy_rate >= 0.7 ? 'var(--green-600)' : student.accuracy_rate >= 0.4 ? 'var(--gold-600)' : 'var(--red-400)',
                    fontWeight: 500,
                  }}>
                    {Math.round(student.accuracy_rate * 100)}%
                  </span>
                </td>
                <td style={{ padding: '12px 20px', color: 'var(--text-secondary)' }}>{student.streak}d</td>
                <td style={{ padding: '12px 20px', color: 'var(--text-secondary)' }}>{student.xp}</td>
                <td style={{ padding: '12px 20px' }}>
                  <Link
