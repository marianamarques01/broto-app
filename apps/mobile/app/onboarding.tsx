    ...
    <View style={st.dotsRow}>
      {Array.from({ length: total }, (_, i) => (
        <View
          ...
          style={[
            st.dot,
            i < current ? st.dotCompleted : i === current ? st.dotActive : st.dotInactive,
          ]}
        />
      ))}
    </View>
...
function StepWelcome({ onContinue, onSkipAll }: { onContinue: () => void; onSkipAll: () => void }) {
  return (
        ...
        <View style={st.chipsRow}>
          {CURSOS_POPULARES.map((c) => {
            ...
            return (
              ...
              >
                <Text style={[st.chipText, selected && st.chipTextSelected]}>{c}</Text>
              </Pressable>
            )
          })}
        </View>
    ...
    >
      ...
      <Animated.View entering={FadeInUp.delay(350).duration(400)} style={st.metaPills}>
        {[500, 600, 700, 750, 800, 850].map((n) => (
          ...
          >
            <Text style={[st.metaPillText, data.metaNota === n && { color: getNotaColor(n) }]}>
              {n}
            </Text>
          </Pressable>
        ))}
      ...
      <Animated.View entering={FadeInUp.delay(450).duration(400)} style={st.refCard}>
        ...
        {NOTAS_REFERENCIA.map((r) => {
          ...
          return (
            <View key={r.curso} style={[st.refRow, isHighlight && st.refRowHighlight]}>
              <Text style={[st.refCurso, isHighlight && st.refCursoHighlight]}>{r.curso}</Text>
              <Text style={[st.refNota, { color: getNotaColor(r.nota) }]}>~{r.nota}</Text>
            </View>
          )
        })}
      </Animated.View>
              ...
              {NIVEIS.map((n) => {
                ...
                return (
                  ...
                  >
                    ...
                    <Text style={[st.nivelText, isSelected && { color: area.color }]}>
                      {n.label}
                    </Text>
                  </Pressable>
                )
        ...
        <View style={st.horasPills}>
          {HORAS_OPTIONS.map((h) => {
            ...
            return (
              ...
              >
                <Text style={[st.horasPillText, selected && st.horasPillTextSelected]}>{h}h</Text>
              </Pressable>
            )
          })}
          <Pressable
          ...
          >
            <Text style={[st.horasPillText, data.horasPorDia >= 6 && st.horasPillTextSelected]}>
              6h+
            </Text>
          </Pressable>
        </View>
          ...
          {HORARIOS.map((h) => {
            ...
            return (
              ...
              >
                ...
                <View
                  style={[st.horarioIconWrap, selected && { backgroundColor: colors.green.glow }]}
                >
                  <h.Icon size={22} color={selected ? colors.green[400] : colors.text.muted} />
                </View>
                <Text style={[st.horarioLabel, selected && { color: colors.text.primary }]}>
                  {h.label}
                </Text>
              </Pressable>
            )
      ...
      <Animated.View entering={FadeInUp.delay(350).duration(400)} style={st.resumoAreasRow}>
        {AREAS.map((area) => (
          <View key={area.key} style={st.resumoAreaCard}>
            ...
            <Text style={st.resumoAreaLabel}>{nivelLabel(data.niveis[area.key])}</Text>
          </View>
        ))}
      </Animated.View>
